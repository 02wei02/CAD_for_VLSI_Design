...
struct wire_	{
    ...
    const char *logic;	/* Name of the wire logic function */
...
int translate_wiretype(char *type)

{
    if (strncmp("inpt",type,4)==0) return INPUT;
    if (strncmp("and",type,4)==0) return AND;
    if (strncmp("nand",type,4)==0) return NAND;
    if (strncmp("or",type,4)==0) return OR;
    if (strncmp("nor",type,4)==0) return NOR;
    if (strncmp("xor",type,4)==0) return XOR;
    if (strncmp("xnor",type,4)==0) return XNOR;
    if (strncmp("buff",type,4)==0) return BUFF;
    if (strncmp("not",type,4)==0) return NOT;
    if (strncmp("from",type,4)==0) return FROM;
